...
function screenshotInspiredBudgetAlertEmailHtml(input: {
  ...
  const inboxSafe = base
    ...
    .replace("▰ &nbsp;Expense Tracker AI", '<img src="__LOGO_DARK_SRC__" width="28" height="28" alt="Expense Tracker AI" style="vertical-align:middle;border:0;border-radius:7px;">&nbsp; Expense Tracker AI')
    .replace("🔷 &nbsp;<b>Expense Tracker AI</b>", '<img src="__LOGO_LIGHT_SRC__" width="22" height="22" alt="Expense Tracker AI" style="vertical-align:middle;border:0;border-radius:6px;">&nbsp; <b>Expense Tracker AI</b>');
  return inboxSafe
...
export function buildServer(store: ExpenseStore, userId: string): McpServer {
  ...
  server.registerTool("add_income", { title: "Add income", description: "Record income for cash-flow and savings reports.", inputSchema: TOOL_INPUTS.add_income }, async ({ amount, source, date, currency, notes }) => {
  ...
  server.registerTool("set_recurring_expense", { title: "Set recurring expense", description: "Create or update a recurring rent, subscription, utility, or loan expense.", inputSchema: TOOL_INPUTS.set_recurring_expense }, async ({ amount, category, description, merchant, frequency, next_date, currency, active }) => {
  ...
  server.registerTool("split_expense", { title: "Split expense", description: "Record one purchase split across two or more categories.", inputSchema: TOOL_INPUTS.split_expense }, async ({ total_amount, date, currency, merchant, description, splits }) => {
    const d = date ?? todayISO(); const cur = currencyOrError(currency);
  ...
  server.registerTool("import_expenses", { title: "Import expenses from CSV", description: "Import CSV columns amount, category, date, description, currency; duplicate rows can be skipped.", inputSchema: TOOL_INPUTS.import_expenses }, async ({ csv, on_duplicate }) => {
  ...
  server.registerTool("manage_categories", { title: "Manage categories", description: "List, create, update, or remove custom category limits and colors.", inputSchema: TOOL_INPUTS.manage_categories }, async ({ action, category, limit, currency, color }) => {
  ...
  server.registerTool("set_alert_thresholds", { title: "Set budget alert thresholds", description: "Set the percentage thresholds that trigger budget alerts.", inputSchema: TOOL_INPUTS.set_alert_thresholds }, async ({ thresholds }) => {
  ...
  server.registerTool("get_cash_flow_report", { title: "Get cash-flow report", description: "Report income, expenses, net cash flow, and savings rate for a month.", inputSchema: TOOL_INPUTS.get_cash_flow_report }, async ({ month }) => {
  ...
  server.registerTool("get_spending_forecast", { title: "Get spending forecast", description: "Forecast month-end spending and remaining daily budget from spending so far.", inputSchema: TOOL_INPUTS.get_spending_forecast }, async ({ month }) => {
  ...
  server.registerTool("compare_months", { title: "Compare monthly spending", description: "Compare expenses across recent months, with a trend chart data series.", inputSchema: TOOL_INPUTS.compare_months }, async ({ months, currency }) => {
  ...
  server.registerTool("get_budget_alerts", { title: "Get budget alerts", description: "List configured threshold alerts and over-budget categories.", inputSchema: TOOL_INPUTS.get_budget_alerts }, async ({ month }) => {
  ...
  server.registerTool("find_duplicate_expenses", { title: "Find duplicate expenses", description: "Find likely duplicate expenses with the same date, category, amount, currency, and description.", inputSchema: TOOL_INPUTS.find_duplicate_expenses }, async ({ from, to }) => {
  ...
  server.registerTool("manage_budget_templates", { title: "Manage budget templates", description: "Save, apply, list, or delete reusable student, family, business, or custom budget templates.", inputSchema: TOOL_INPUTS.manage_budget_templates }, async ({ action, name, template }) => {
  ...
  server.registerTool(
    ...
    async ({ format, from, to, limit, offset }) => {
      ...
      if (format === "pdf") {
        const pdf_base64 = reportPdfBase64("Expense Tracker Export", items.map((item) => `${item.date} | ${item.category} | ${item.amount.toFixed(2)} ${item.currency} | ${item.description}`));
